...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let doCalls = (candidates) =>
    new Promise((resolve, reject) => {
      callLines.forEach((callLine) => (callLine.textContent = ''));

      if (candidates.length == 0) {
        resultLine.textContent = 'Никто не победил...';
        reject('Никто не победил...'); //верните неудачный результат главного промиса
        ...
        let callPromises = callLines.map((callLine) => {
          ...
          if (nextCandidate && candidates.length > 0) {
            //номера ещё остались
            return doCall(callLine, nextCandidate); // сделайте вызов
          } else if (nextCandidate && candidates.length == 0) {
            // победитель
        ...
        Promise.any(callPromises)
          .then((candidate) => {
            //здесь надо дождаться первого кто ответит (то есть выполнит промис)
            candidates.push(candidate);
          })
          .catch(() => {
            console.log('no candidates now');
          });
        Promise.allSettled(callPromises).then(() => {
          //а здесь надо дождаться выполнения  всех промисов
