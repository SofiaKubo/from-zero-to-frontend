...
export function renderTasks(tasks) {
  ...
  tasks.forEach((task) => {
    ...
    taskElement.querySelector('.task-priority').textContent = `Приоритет: ${task.priority}`;
    taskElement.querySelector('.task-deadline').textContent =
...
export function renderNews(news) {
  ...
  news.forEach((item) => {
    ...
    newsElement.querySelector('.news-author').textContent = `Автор: ${item.author}`;
